...
func main() {
	...
	if *spiffeID != "" && !*spiffeWorkloadAPI && *spiffeBundle == "" && *cacert == "" {
		fail(nil, "The -spiffe-id argument requires a trust source: use -cacert, -spiffe-bundle, or -spiffe-workload-api.")
	}
		...
		if *plaintext {
			...
			if *spiffeID != "" {
				...
				if *spiffeWorkloadAPI {
					tlsConf, err = grpcurl.SpiffeConfigFromWorkloadAPI(ctx, *spiffeID, "")
				} else if *spiffeBundle != "" {
					tlsConf, err = grpcurl.SpiffeConfigFromBundleMap(*spiffeID, *spiffeBundle, *cert, *key)
				} else {
					tlsConf, err = grpcurl.SpiffeConfigFromCAFiles(*spiffeID, *cacert, *cert, *key)
				}
				if err != nil {
