...
func TestSPIFFETLS_Valid(t *testing.T) {
	...
	tlsConf, err := SpiffeConfigFromCAFiles(spiffeURI, "internal/testing/tls/ca.crt", "", "")
	if err != nil {
...
func TestSPIFFETLS_WrongID(t *testing.T) {
	...
	tlsConf, err := SpiffeConfigFromCAFiles(clientExpectedURI, "internal/testing/tls/ca.crt", "", "")
	if err != nil {
		t.Fatalf("create SPIFFE TLS config: %v", err)
	}
	...
	if !strings.Contains(dialErr.Error(), "unexpected ID") {
...
func TestSPIFFETLS_NoURISAN(t *testing.T) {
	...
	tlsConf, err := SpiffeConfigFromCAFiles("spiffe://example.org/myservice", "internal/testing/tls/ca.crt", "", "")
	if err != nil {
		t.Fatalf("create SPIFFE TLS config: %v", err)
	}
	...
	if !strings.Contains(dialErr.Error(), "could not get leaf SPIFFE ID") {
		t.Fatalf("expected SPIFFE verification error for missing URI SAN, got: %v", dialErr)
	}
}
...
func TestSPIFFETLS_UntrustedCA(t *testing.T) {
	...
	tlsConf, err := SpiffeConfigFromCAFiles(spiffeURI, "internal/testing/tls/wrong-ca.crt", "", "")
	if err != nil {
		t.Fatalf("create SPIFFE TLS config: %v", err)
	}
	...
	if !strings.Contains(dialErr.Error(), "unknown authority") {
		t.Fatalf("expected untrusted CA error, got: %v", dialErr)
	}
}
